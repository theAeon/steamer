...
RUN pip wheel "ncls==0.0.68" "pandas==1.5.3" "sorted-nearest==0.0.39" "pybedtools==0.10.0"

FROM python:3.12-slim

RUN apt-get update && apt-get upgrade -y && apt-get install curl gpg git bedtools libhdf5-103-1 tabix --no-install-recommends -y
...
RUN apt-get update && apt-get install google-cloud-cli --no-install-recommends -y
...
COPY --from=builder /src/pybedtools-0.10.0-cp312-cp312-linux_x86_64.whl /src/pybedtools-0.10.0-cp312-cp312-linux_x86_64.whl
